...
def find_most_visited_day():
    """
    Traverses the table and counts the times each day is located.
    ...
    for visit in table:
        if visit != 0:
            counters[days.index(visit[2])] += 1
    return days[counters.index(max(counters))]
...
start_t = time.time()
day = find_most_visited_day()
print("Finding most visited day took {} seconds".format(time.time() - start_t))
print("The most visited day was", day)
